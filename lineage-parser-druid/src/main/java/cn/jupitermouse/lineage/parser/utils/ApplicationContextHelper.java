package cn.jupitermouse.lineage.parser.utils;
        ...
        executorService.scheduleAtFixedRate(() -> {
            ...
            if (success) {
                ...
                if (counter.addAndGet(1) > 240) {
                    LOGGER.error("Setter field [{}] in [{}] failure because timeout.", setterMethod,
    ...
    private static boolean setByMethod(Class<?> type, Object target, String targetMethod) {
        if (ApplicationContextHelper.getContext() != null) {
            try {
                ...
                LOGGER.info("Async set field [{}] in [{}] success by method.", targetMethod,
                        target.getClass().getName());
                return true;
                ...
                LOGGER.error("Async set field [{}] in [{}] failure by method.", targetMethod,
